#!/usr/bin/env python3
"""
TrueType Font (TTF) Format Extractor

Extracts comprehensive structure from TrueType font files (.ttf).

TrueType fonts are binary files containing:
1. Offset Table (12 bytes): sfnt version, table count, search parameters
2. Table Directory (16 bytes per table): tag, checksum, offset, length
3. Font Tables: Required and optional tables with specific data

Required Tables:
- head: Font header (metadata, units per em, bounding box)
...
- GDEF: Glyph definition data (OpenType)
- GPOS: Glyph positioning data (OpenType)
- GSUB: Glyph substitution data (OpenType)
- kern: Kerning data
...
class TTFExtractor:
    """Extract structure and metadata from TrueType Font files."""
    
    def __init__(self):
        """Initialize TTF extractor."""
        self.file_path = None
        self.file_size = 0
        self.data = None
    ...
    def extract(self, file_path: str) -> Dict[str, Any]:
        """
        Extract complete structure from TTF file.
        
        Args:
            file_path: Path to TTF file
    ...
    def _read_offset_table(self):
        ...
        # Offset table structure (12 bytes):
        # uint32 sfntVersion: 0x00010000 for TrueType, 'OTTO' for CFF/OpenType
        ...
        if sfnt_version == 0x00010000:
            font_type = 'TrueType'
        elif sfnt_version == 0x4F54544F:  # 'OTTO'
    ...
    def _read_table_directory(self):
        ...
        # Table directory starts at byte 12
        ...
        offset = 12  # Start after offset table
    ...
    def _read_tables(self):
        ...
        if 'loca' in self.table_directory:
            self.tables['loca'] = self._read_loca_table()
        
        if 'glyf' in self.table_directory:
            self.tables['glyf'] = self._read_glyf_table()
        
        # Optional tables
        if 'kern' in self.table_directory:
            self.tables['kern'] = self._read_kern_table()
    ...
    def _read_head_table(self) -> Dict[str, Any]:
        ...
        # head table structure (54 bytes minimum)
        # Fixed version: 0x00010000
        # Fixed fontRevision
        # uint32 checkSumAdjustment
        # uint32 magicNumber: 0x5F0F3CF5
        # uint16 flags
        # uint16 unitsPerEm: 16-2048, typically 1000 or 2048
        ...
        return {
            ...
            'index_to_loc_format': 'short (Offset16)' if index_to_loc_format == 0 else 'long (Offset32)',
            'index_to_loc_format_value': index_to_loc_format,
    ...
    def _read_name_table(self) -> Dict[str, Any]:
        ...
        # uint16 stringOffset: Offset to start of string storage
        # NameRecord[count]: Array of name records
        ...
        for i in range(count):
            ...
            # Store primary English name
            if platform_id == 3 and language_id == 0x0409:  # Windows, English US
                names[name_key] = text
        ...
        return {
            ...
            'count': count,
            'names': names,
            'records': name_records[:10]  # Limit to first 10 for brevity
        }
    ...
    def _read_maxp_table(self) -> Dict[str, Any]:
        ...
        # Fixed version: 0x00010000 (TrueType) or 0x00005000 (CFF)
        # uint16 numGlyphs: Number of glyphs
        # For TrueType (version 1.0), additional fields...
        ...
        # If version 1.0 (TrueType), read additional fields (need 32 bytes total)
        if version == 0x00010000 and entry['length'] >= 32:
            # Read 13 uint16 values (26 bytes) from offset+6 to offset+32
    ...
    def _read_hmtx_table(self) -> Dict[str, Any]:
        ...
        # Get number of glyphs and metrics from other tables
        ...
        # hmtx table structure:
        # longHorMetric[numOfLongHorMetrics]: advanceWidth + lsb
        # FWord[numGlyphs - numOfLongHorMetrics]: Additional lsb values
        
        metrics = []
        pos = offset
        
        # Read long metrics (first numOfLongHorMetrics glyphs)
        for i in range(min(num_long_metrics, 20)):  # Limit to first 20
        ...
        return {
            ...
            'num_glyphs': num_glyphs,
            'metrics': metrics  # First 20 glyphs only
        }
    ...
    def _read_os2_table(self) -> Dict[str, Any]:
        ...
        # OS/2 table has multiple versions (0-5), sizes vary
        version = struct.unpack('>H', self.data[offset:offset+2])[0]
        
        # Common fields across all versions (first 68 bytes)
        if length >= 68:
    ...
    def _read_post_table(self) -> Dict[str, Any]:
        ...
        # post table structure (minimum 32 bytes):
        # Fixed version
        # Fixed italicAngle
    ...
    def _read_cmap_table(self) -> Dict[str, Any]:
        ...
        for i in range(min(num_tables, 10)):  # Limit to first 10
    ...
    def _read_loca_table(self) -> Dict[str, Any]:
        """Read 'loca' table (index to location)."""
        entry = self.table_directory['loca']
        offset = entry['offset']
        length = entry['length']
        
        # Get format from head table
        head_table = self.tables.get('head', {})
        format_value = head_table.get('index_to_loc_format_value', 1)
        
        # Get number of glyphs
        num_glyphs = self.tables.get('maxp', {}).get('num_glyphs', 0)
        
        if num_glyphs == 0:
            return {'error': 'Missing maxp table'}
        
        # loca table contains offsets to glyph data
        # Format 0: uint16 offsets (divided by 2)
        # Format 1: uint32 offsets
        
        offsets = []
        
        if format_value == 0:  # Short format
            num_entries = min(length // 2, num_glyphs + 1, 20)
            for i in range(num_entries):
                offset_value = struct.unpack('>H', self.data[offset+i*2:offset+i*2+2])[0]
                offsets.append(offset_value * 2)
        else:  # Long format
            num_entries = min(length // 4, num_glyphs + 1, 20)
            for i in range(num_entries):
                offset_value = struct.unpack('>I', self.data[offset+i*4:offset+i*4+4])[0]
                offsets.append(offset_value)
        
        return {
            'format': 'short (Offset16)' if format_value == 0 else 'long (Offset32)',
            'num_offsets': len(offsets),
            'offsets': offsets  # First 20 only
        }
    
    def _read_glyf_table(self) -> Dict[str, Any]:
        """Read 'glyf' table (glyph data/outlines)."""
        entry = self.table_directory['glyf']
        
        # glyf table contains actual glyph outline data
        # Each glyph has:
        # - Header (10 bytes): numberOfContours, xMin, yMin, xMax, yMax
        # - Outline data (variable length)
        
        # We'll just report basic info without parsing all glyphs
        return {
            'offset': entry['offset'],
            'length': entry['length'],
            'note': 'Glyph outline data (TrueType curves)',
            'parsed': False,
            'reason': 'Complex glyph outline parsing requires detailed curve analysis'
        }
    
    def _read_kern_table(self) -> Dict[str, Any]:
        """Read 'kern' table (kerning data)."""
        entry = self.table_directory['kern']
        offset = entry['offset']
        
        # kern table structure:
        # uint16 version
        # uint16 nTables
        
        version = struct.unpack('>H', self.data[offset:offset+2])[0]
        n_tables = struct.unpack('>H', self.data[offset+2:offset+4])[0]
        
        return {
            'version': version,
            'num_tables': n_tables,
            'note': 'Kerning pairs for glyph positioning'
        }
    ...
    def _compute_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_kerning': 'kern' in self.table_directory or 'GPOS' in self.table_directory,
            'has_opentype_features': any(tag in self.table_directory for tag in ['GPOS', 'GSUB', 'GDEF'])
        }


def main():
    """Test TTF extractor with command-line arguments."""
    import sys
    import json
    
    if len(sys.argv) != 2:
        print("Usage: python3 ttf_extractor.py <font_file.ttf>")
        sys.exit(1)
